...
import { InteractiveQuestionHelper } from './InteractiveQuestionHelper';
...
export default function StudentGame({ questions, onExit, context, onFinished }: Props) {
  ...
  const [showHelper, setShowHelper] = useState(false); // Show animated visual helper

  // Get current question with options ensured
  const rawCurrent = questions[index];
  const current = useMemo(() => {
    return rawCurrent ? ensureQuestionHasOptions(rawCurrent) : undefined;
  }, [rawCurrent]);
  ...
  useEffect(() => {
    ...
    setShowHelper(false); // Reset visual helper
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-xl px-4 py-6 md:py-8">
        ...
        {/* Question card - Always show multiple choice options */}
        ...
        {/* Interactive Visual Helper - Animated step-by-step explanation */}
        <div className="mt-4">
          <InteractiveQuestionHelper
            question={current}
            locale={locale}
            isExpanded={showHelper}
