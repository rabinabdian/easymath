...
export function QuestionCard({
  ...
  const textAlignmentClass = locale === 'he' ? 'text-right' : 'text-left';
      ...
      <div className="flex flex-1 flex-col gap-3">
        {question.isReadingExercise ? (
          ...
          <div className="flex flex-col items-center gap-4 p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl">
            <SpeakerButton text={questionText} size="large" variant="primary" label={speakerLabel} />
            <p className="text-lg font-semibold text-slate-900 text-center whitespace-pre-line">
              {questionText}
            </p>
          </div>
        ) : (
          // Regular question - Show text with inline speaker icon
          <div className="flex items-start justify-between gap-3">
            <p className={`text-lg font-semibold text-slate-900 whitespace-pre-line flex-1 ${textAlignmentClass}`}>
              {questionText}
            </p>
